<?php
  ...
  $join = "SELECT * FROM mahasiswa LEFT JOIN kelas ON mahasiswa.kelas_id = kelas.kelas_id";
  $joinkelas = $conn->query($join);
      ...
      <div class="row">
        <div class="col-md-4 order-md-2 mb-4">
          <h4 class="d-flex justify-content-between align-items-center mb-3">
            ...
            <span class="badge badge-secondary badge-pill"><?= mysqli_num_rows($data_mahasiswa) ?></span>
          </h4>
          <?php foreach($joinkelas as $index=>$value): ?>
            <ul class="list-group mb-3">
              <li class="list-group-item d-flex justify-content-between lh-condensed">
                <div>
                  <h6 class="my-0"><?= $value['nama_lengkap'] ?></h6>
                  <small class="text-muted"><?= $value['alamat'] ?></small>
                </div>
                <span class="text-muted"><?= $value['nama'] ?></span>
                <a class="close" href="hapus_data.php?mahasiswa_id=<?= $value['mahasiswa_id'] ?>" type="button">
                  <span class="fa fa-trash"></span>
                </a>
                <a class="close" href="update_form.php?mahasiswa_id=<?= $value['mahasiswa_id'] ?>" type="button">
        ...
        <h4 class="mb-3">Input Data</h4>
          ...
          <input type="hidden" name="mahasiswa_id" value="<?= $data_select_mahasiswa['mahasiswa_id'] ?>">
            <div class="mb-3">
              <label for="nama_lengkap">Nama Lengkap</label>
              <input type="text" class="form-control" id="nama_lengkap" name="nama_lengkap" value="<?= $data_select_mahasiswa['nama_lengkap'] ?>" required>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
              ...
              <select class="custom-select d-block w-100" id="Kelas" name="kelas_id" required>
                ...
                <?php foreach($data_kelas as $index => $value): ?>
                  ...
                  ?>
                  <option <?= $is_selected ?> value="<?= $value['kelas_id'] ?>"><?= $value['nama'] ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="mb-3">
